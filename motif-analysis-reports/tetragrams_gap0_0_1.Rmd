---
title: "Tetragrams with gaps 0, 0, 1 analysis"
...
trigram_freqs <- calculate_ngram_freq(filtered_datasets_list, 4, list(c(0, 0, 1)))
...
### Numbers of motifs with given frequency
```{r}
trigram_plot_dat %>% 
  filter(frequency > 0) %>% 
...
if(nrow(filter(motif_counts_datasets, n_datasets == 9))) {
...
## Detailed analysis of presequences
```{r}
trigram_fracs <- calculate_ngram_fractions(presequence_datasets_list, 4, list(c(0, 0, 1)))
...
binary_trigrams <- lapply(names(presequence_datasets_list), function(ith_dataset) {
  calculate_binary_ngram_matrix(presequence_datasets_list[[ith_dataset]], 4, list(c(0, 0, 1))) %>%
...
save(test_res, file = "trigrams01_test_res.rda")
