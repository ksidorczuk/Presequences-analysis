...
add_group <- function(df) {
  mutate(df,
         group = case_when(dataset %in% c("Amyloids combined", "AmyPro regions", "CPAD peptides") ~ "Amyloids",
                           ...
                           dataset %in% c("cTP-mTP experimentally verified location", "cTP-mTP experimentally verified presequence",
                                          "dTP experimentally verified location", "dTP experimentally verified presequence") ~ "dTP",
...
results_path <- "~/Dropbox/Presequences/Clustering_grouped_datasets/"

colors <- c("AmyPro regions" = "#e44496", "CPAD peptides" = "#8d44e4", 
            "cTP experimentally verified location" = "#528313", "dTP experimentally verified location" = "#837713", 
            "dTP experimentally verified presequence" ="#e4d345", "cTP experimentally verified presequence" = "#9de444", 
...
colors_short <- c("Amyloids" = "#bb45e4", "dTP" ="#e4d345", "cTP" = "#9de444", 
...
datasets_list_reduced <- list("Amyloids" = datasets_list_cdhit[["Amyloids combined"]],
                              "dTP" = c(datasets_list_cdhit[["cTP-mTP experimentally verified location"]], datasets_list_cdhit[["cTP-mTP experimentally verified presequence"]])[which(!duplicated( c(datasets_list_cdhit[["cTP-mTP experimentally verified location"]], datasets_list_cdhit[["cTP-mTP experimentally verified presequence"]])))],
...
names(datasets_list) <- c("AmyPro regions", "CPAD peptides", "cTP experimentally verified location", 
                          "dTP experimentally verified location", "dTP experimentally verified presequence", 
                          "cTP experimentally verified presequence", "DBAASP AMP", "DBAASP AMP max 100 aa", 
                          "mTP experimentally verified location", "mTP experimentally verified presequence", 
                          "SP experimentally verified presequence", "TM regions experimentally verified - alpha", 
                          "TM regions experimentally verified - beta")
datasets_list_cdhit <- datasets_list_cdhit[which(names(datasets_list_cdhit) != "Amyloids combined")]
names(datasets_list_cdhit) <- c("AmyPro regions", "CPAD peptides", "cTP experimentally verified location", 
                          "dTP experimentally verified location", "dTP experimentally verified presequence", 
                          "cTP experimentally verified presequence", "DBAASP AMP", "DBAASP AMP max 100 aa", 
                          "mTP experimentally verified location", "mTP experimentally verified presequence", 
                          "SP experimentally verified presequence", "TM regions experimentally verified - alpha", 
...
x <- len_df %>% 
  filter(dataset != "DBAASP AMP max 100 aa") %>% 
  ggplot(aes(x = dataset, y = length, fill = dataset)) +
  geom_violin() +
  ...
  theme(legend.position = "none", 
        ...
        axis.title.x = element_blank())
x
ggsave(paste0(results_path, "Lengths.eps"), x, width = 12, height = 14)
```
```{r fig.height = 12}
ggplot(len_df, aes(x = dataset, y = length, fill = dataset)) +
  geom_violin() +
...
p <- all_combns %>% 
  ...
  mutate(Comparison = gsub(" experimentally verified", "", Comparison)) %>%
  mutate(Comparison = gsub("presequence", "(presequence)", Comparison)) %>% 
  mutate(Comparison = gsub("location", "(location)", Comparison)) %>% 
  ...
  theme(panel.grid.major = element_blank()) +
  scale_fill_manual("Is significant", values = c(`FALSE` = "#76bef2", `TRUE` = "#ff4242")) 
ggsave(paste0(results_path, "Test_aa_comp.eps"), p, width = 9, height = 14)
p
```
...
p <- ggbiplot(pca_res_all, choices = 1:2) +
  theme_bw() +
  ...
  theme(legend.position = "none")
ggsave(paste0(results_path, "PCA_aa_comp.eps"), p, width = 10, height = 12)
```
...
props <- lapply(seq_along(datasets_list_cdhit), function(i) {
  ...
  setNames(c("Residue volume", "Polarizability parameter", "Frequency of alpha-helix",
             "Frequency of beta-sheet", "Frequency of beta-turn",
             "van der Waals volume", "Net charge", "Hydropathy index", "Polarity"))
...
props_long <- props %>%
  mutate(Dataset = datasets_cdhit) %>%
  pivot_longer(1:9, names_to = "Property", values_to = "Value")

ggplot(props_long, aes(x = Dataset, y = Value, fill = Dataset)) +
  geom_violin() +
...
### Statistical testing of physicochemical properties distribution
```{r}
combns <- combn(sort(unique(props_long[["Dataset"]])), 2, simplify = FALSE)
props_test_res <- lapply(unique(props_long[["Property"]]), function(ith_prop) {
  lapply(1:length(combns), function(i) {
    data.frame(Dataset1 = combns[[i]][1],
               Dataset2 = combns[[i]][2],
               Property = ith_prop,
               pval = wilcox.test(x = filter(props_long, Property == ith_prop, Dataset == combns[[i]][1])[["Value"]],
                                  y = filter(props_long, Property == ith_prop, Dataset == combns[[i]][2])[["Value"]],
                                  exact = FALSE)[["p.value"]])
  }) %>% bind_rows()
}) %>% bind_rows() %>%
    mutate(pval_adjusted = p.adjust(pval)) %>%
    select(-pval) %>%
    group_by(Property, Dataset1, Dataset2) %>%
    dplyr::summarise(is_signif = pval_adjusted < 0.05) 

ggplot(props_test_res, aes(x = Dataset1, y = Dataset2, fill = is_signif)) +
  geom_tile(color = "white") +
  facet_wrap(~Property) +
  theme_bw() +
  theme(panel.grid.major = element_blank(),
...
ggplot(props_long, aes(x = Dataset, y = Value, fill = Dataset)) +
  geom_violin() +
  facet_wrap(~Property) +
  scale_fill_manual("Data set", values = colors_short) +
  theme_bw(base_size = 14) +
  xlab("") +
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank(),
...
p_all <- ggbiplot(pca_props_res1, choices = 1:2, alpha = 0.25, ellipse = TRUE, groups = datasets) +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_color_manual("Data set", values = colors_short)
ggsave(paste0(results_path, "PCA_props_grouped_all.eps"), p_all, width = 10, height = 10)
p_all

p <- ggbiplot(pca_props_res1, choices = 1:2, alpha = 0.25, ellipse = TRUE, groups = datasets) +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5),
        legend.position = "none") +
  scale_color_manual("Data set", values = colors_short) +
  facet_wrap(~groups, ncol = 4) + 
    xlim(-8, 12) +
    ylim(-12, 12)
ggsave(paste0(results_path, "PCA_props_grouped.eps"), p, width = 15, height = 10)
p

pca_prop_res_amp_tp <- pca_props_res1
pca_prop_res_amp_tp[["x"]] <- pca_prop_res_amp_tp[["x"]][which(datasets %in% c("AMP", "cTP", "dTP", "mTP")),]
pca_prop_res_amp_tp[["x"]] <- pca_prop_res_amp_tp[["x"]][order(nrow(pca_prop_res_amp_tp[["x"]]):1),]

p2 <- ggbiplot(pca_prop_res_amp_tp, choices = 1:2, alpha = 0, ellipse = TRUE, groups = rev(datasets[which(datasets %in% c("AMP", "cTP", "dTP", "mTP"))])) +
  theme_bw() +
  geom_point(data = mutate(as.data.frame(pca_prop_res_amp_tp[["x"]]),
                           dataset = rev(datasets[which(datasets %in% c("AMP", "cTP", "dTP", "mTP"))])), 
             aes(x = PC1, y = PC2, color = dataset), alpha = 0.25) +
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_color_manual("Data set", values = c(dTP = "#e4d345", cTP = "#9de444", 
                                            AMP = "grey70", mTP = "#e49144"))
ggsave(paste0(results_path, "PCA_props_grouped_AMP-TP.pdf"), p2, width = 12, height = 10)


pca_prop_res_amp_sp <- pca_props_res1
pca_prop_res_amp_sp[["x"]] <- pca_prop_res_amp_sp[["x"]][which(datasets %in% c("AMP", "SP")),]
pca_prop_res_amp_sp[["x"]] <- pca_prop_res_amp_sp[["x"]][order(nrow(pca_prop_res_amp_sp[["x"]]):1),]

p3 <- ggbiplot(pca_prop_res_amp_sp, choices = 1:2, alpha = 0, ellipse = TRUE, groups = rev(datasets[which(datasets %in% c("AMP", "SP"))])) +
  theme_bw()  +
  geom_point(data = mutate(as.data.frame(pca_prop_res_amp_sp[["x"]]),
                           dataset = rev(datasets[which(datasets %in% c("AMP", "SP"))])), 
             aes(x = PC1, y = PC2, color = dataset), alpha = 0.25) +
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_color_manual("Data set", values = c(AMP = "grey70", SP = "#45e495"))
ggsave(paste0(results_path, "PCA_props_grouped_AMP-SP.pdf"), p3, width = 12, height = 10)
```
...
p <- pca_bigram_res1[["x"]] %>%
  ...
  scale_color_manual("Data set", values = colors_short) +
  theme_bw()
ggsave(paste0(results_path, "PCA_bigrams_grouped.png"), p, width = 12, height = 10)
pca_bigram_res1[["x"]] %>%
  as.data.frame() %>%
  mutate(dataset = datasets) %>%
  ggplot(aes(x = PC1, y = PC2, color = dataset)) +
...
```{r}
datasets_list_exp <- list("Amyloids" = datasets_list_reduced[["Amyloids"]],
                          "dTP" = datasets_list_cdhit[["dTP experimentally verified presequence"]],
                          ...
                          "TM alpha" = datasets_list_cdhit[["TM regions experimentally verified - alpha"]],
                          "TM beta" = datasets_list_cdhit[["TM regions experimentally verified - beta"]])
datasets_exp <- unlist(sapply(names(datasets_list_exp), function(i) rep(i, length(datasets_list_exp[[i]])), USE.NAMES = FALSE))
```
...
p <- umap_res[["layout"]] %>%
  ...
  scale_color_manual("Data set", values = colors_short) +
  theme_bw()
ggsave(paste0(results_path, "UMAP_aa_comp_exp.eps"), p, width = 12, height = 9)
```
...
p <- ggbiplot(pca_props_res, choices = 1:2, alpha = 0.25, ellipse = TRUE, groups = datasets_exp) +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5),
        legend.position = "none") +
  scale_color_manual("Data set", values = colors_short) +
  facet_wrap(~groups, ncol = 4)  + 
    xlim(-8, 12) +
    ylim(-12, 12)
ggsave(paste0(results_path, "PCA_props_exp.png"), p, width = 10, height = 12)
```
