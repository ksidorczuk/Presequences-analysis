---
title: "Presequences + AMP: Trigrams with gaps 3 and 0 analysis"
...
trigram_freqs <- calculate_ngram_freq(filtered_datasets_list, 3, list(c(3, 0)))
...
binary_trigrams <- lapply(names(presequence_datasets_list), function(ith_dataset) {
  calculate_binary_ngram_matrix(presequence_datasets_list[[ith_dataset]], 3, list(c(3, 0))) %>%
...
binary_trigrams <- lapply(names(amp_datasets_list), function(ith_dataset) {
  calculate_binary_ngram_matrix(amp_datasets_list[[ith_dataset]], 3, list(c(3, 0))) %>%
    mutate(dataset = ith_dataset)
}) %>% bind_rows()
binary_trigrams[is.na(binary_trigrams)] <- 0
```
